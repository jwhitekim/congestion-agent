...
기존 Django 기반 혼잡도 분석 시스템(학부 캡스톤)을 training-free, prompting 기반 순수 Python 패키지로 리팩토링한 것.
...
```bash
python -m agent.main samples/supermarket.mp4
```

세그먼트마다 터미널에 PERCEPTION / TRIGGER / AGENT 3줄이 출력되고, 전체 결과는 `results.json`에 누적 저장된다 (트리거 안 걸린 세그먼트도 `agent: null`로 포함, tool_raw 등 연구용 원본 데이터 보존).

### 임계값 보정

```bash
python scripts/calibrate.py samples/supermarket.mp4
```

영상의 density 분포를 percentile로 계산해 `config.py`의 `DENSITY_LOW`/`DENSITY_HIGH` 제안값을 `calibration_thresholds.json`에 저장한다.
...
- LLM 판단(`assessment.action`) 이후 실제로 알림·로깅 등을 실행하는 **action 레이어는 아직 없다.** 현재는 판정 결과를 출력·저장만 한다. 설계는 AGENTS.md 3-1절 참고.
- `tools/`에는 `track_people` 하나만 등록되어 있다.
- 실시간 모니터링 vs. 사후 분석 리포트, 이 시스템의 최종 정체성은 아직 미정.

## 하드웨어

V100 32GB × 2 환경에서 개발. Fine-tuning 없이 순수 prompting 기반 접근만 사용한다.
