...
# (trigger_name, facts) — 서로 다른 crowd size/zone 분포/추세를 갖는 5개 시나리오.
...
PROBES = [
    ("probe_stable", _facts(4, 4.0, 35.0, {"A": 2, "B": 1, "C": 1}, 1.02, 0.01, 0.02, "low")),
    ("surge", _facts(28, 28.0, 20.0, {"A": 9, "B": 10, "C": 9}, 1.6, -0.05, 0.3, "medium")),
    ("hotspot", _facts(37, 20.0, 15.0, {"A": 32, "B": 3, "C": 2}, 1.1, -0.02, 0.1, "medium")),
    ("stagnation", _facts(18, 18.0, 0.2, {"A": 6, "B": 6, "C": 6}, 1.05, -0.15, 0.05, "medium")),
    ("conflict", _facts(12, 12.0, 25.0, {"A": 4, "B": 4, "C": 4}, 1.15, -0.03, 0.9, "low")),
]


def main() -> None:
    results = []
    for trigger_name, facts in PROBES:
        output = agent_loop.run(facts, trigger_name)
